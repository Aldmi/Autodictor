        ...
        private void ОбновитьСостояниеТаблицыШаблонов()
        {
            for (int item = 0; item < this.lVШаблоны.Items.Count; item++)
            {
                if (item <= Record.СписокФормируемыхСообщений.Count)
                {
                    ...
                    var времяПриб = Record.ФиксированноеВремяПрибытия == null ? Record.ВремяПрибытия : Record.ФиксированноеВремяПрибытия.Value;
                    var времяОтпр = Record.ФиксированноеВремяОтправления == null ? Record.ВремяОтправления : Record.ФиксированноеВремяОтправления.Value;
