                    ...
                    if (Данные.Активность == true)
                    {
                       if ((Данные.БитыНештатныхСитуаций & 0x0F) == 0x00)
                        ...
                        for (int j = 0; j < Данные.СписокФормируемыхСообщений.Count; j++)
                        {
                            ...
                            var времяПриб = Данные.ФиксированноеВремяПрибытия == null ? Данные.ВремяПрибытия : Данные.ФиксированноеВремяПрибытия.Value;
                            var времяОтпр = Данные.ФиксированноеВремяОтправления == null ? Данные.ВремяОтправления : Данные.ФиксированноеВремяОтправления.Value;
                        ...
                        if (КоличествоВключенныхГалочек < Данные.СписокФормируемыхСообщений.Count(s=> !s.НазваниеШаблона.StartsWith("@")))
                        {
                        ...
                        if (!Данные.Автомат)
                        {
                            //if (СамоеРаннееВремя.AddMinutes(-10) <= DateTime.Now)
                            {
