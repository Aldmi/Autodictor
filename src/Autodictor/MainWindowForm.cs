    ...
    public partial class MainWindowForm : Form
    {
                    ...
                    switch (шаблон)
                    {
                        ...
                        case "НУМЕРАЦИЯ СОСТАВА":
                            if ((record.НумерацияПоезда > 0) && (record.НумерацияПоезда <= 2))
                            {
                                ...
                                //if ((record.БитыАктивностиПолей & 0x1F) != 0x00)
                                //{
                                //    if (record.СменнаяНумерацияПоезда && формируемоеСообщение.ПривязкаКВремени == 1) // если привязка шаблона к ОТПР, то смена нумерации
                                //    {
                                //        switch (нумерацияПоезда)
                                //        {
                                //            case 1:
                                //                нумерацияПоезда = 2;
                                //                break;
        ...
        private void СохранениеИзмененийДанныхВКарточке(SoundRecord старыеДанные, SoundRecord данные)
        {
            var recChange = new SoundRecordChanges
            {
                ...
                Rec = старыеДанные,
                NewRec = данные,
                UserInfo= $"{Program.AuthenticationService.CurrentUser.Login}  ({Program.AuthenticationService.CurrentUser.Role.ToString()})"
            };
