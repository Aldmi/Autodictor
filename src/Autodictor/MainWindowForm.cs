                    ...
                    if (Данные.Активность == true)
                    {
                        ...
                        if ((Данные.БитыНештатныхСитуаций & 0x0F) != 0x00)
                        {
                            if (Данные.СостояниеКарточки != 6)
                            {
                                ...
                                if ((Данные.БитыНештатныхСитуаций & 0x08) != 0x00) Данные.ОписаниеСостоянияКарточки = "Отправление по готовности поезда";
                            ...
                            if (DateTime.Now < ВременноеВремяСобытия)
                            {
                                ...
                                if ((Данные.БитыНештатныхСитуаций & 0x08) != 0x00)
                                    if (Данные.ТаймерПовторения >= (uint)(Program.Настройки.ИнтервалМеждуОповещениемООтправлениеПоГотовности * 60.0))
                                    {
                                        Данные.ТаймерПовторения = 0;
                ...
                foreach (string шаблон in элементыШаблона)
                {
                    ...
                    switch (шаблон)
                    {
                        ...
                        case "ВРЕМЯ ЗАДЕРЖКИ":
                            ...
                            {
                                воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение { ИмяВоспроизводимогоФайла = ФайлыЧасов[Record.ВремяЗадержки.Value.Hour], Язык = язык });
                                воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение { ИмяВоспроизводимогоФайла = ФайлыМинут[Record.ВремяЗадержки.Value.Minute], Язык = язык });
                            }
                            continue;
                        ...
                        case "ОЖИДАЕМОЕ ВРЕМЯ":
                            ...
                            Text = Record.ВремяОтправления.ToString("HH:mm");
                            logMessage += Text + " ";
                            воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение { ИмяВоспроизводимогоФайла = ФайлыЧасов[Record.ОжидаемоеВремя.Hour], Язык = язык });
                            воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение { ИмяВоспроизводимогоФайла = ФайлыМинут[Record.ОжидаемоеВремя.Minute], Язык = язык });
                            continue;
