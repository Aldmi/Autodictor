...
namespace MainExample
{
    ...
    public struct SoundRecord
    {
        ...
        public DateTime? ФиксированноеВремя;                 //
    ...
    public struct СостояниеФормируемогоСообщенияИШаблон
    {
        ...
        public int? ВремяСмещенияЗафиксированное; // вычислятся в РУЧНОМ режиме при нажатии на кнопку "фиксировать"
                            ...
                            if (ВремяАктивацииШаблона.Length > 0)
                            {
                                for (int j = 0; j < ВремяАктивацииШаблона.Length; j++)
                                {
                                    ...
                                    if ((int.TryParse(ВремяАктивацииШаблона[j], out ВремяСмещения)) == true)
                                    {
                                        ...
                                        НовыйШаблон.ВремяСмещенияЗафиксированное = null;
                        ...
                        for (int j = 0; j < Данные.СписокФормируемыхСообщений.Count; j++)
                        {
                            ...
                            if (!Данные.Автомат)
                            {
                                if (формируемоеСообщение.НазваниеШаблона.StartsWith("@") &&
                                   (формируемоеСообщение.ВремяСмещенияЗафиксированное == null))
                                {
                                    continue;
                                }
                            }

                            var времяСмешения = (формируемоеСообщение.ВремяСмещенияЗафиксированное == null) ? формируемоеСообщение.ВремяСмещения : формируемоеСообщение.ВремяСмещенияЗафиксированное.Value;
                            DateTime времяСобытия = формируемоеСообщение.ПривязкаКВремени == 0 ? Данные.ВремяПрибытия : Данные.ВремяОтправления;
                            времяСобытия = времяСобытия.AddMinutes(времяСмешения);
                            if (j == 0)
                            {
