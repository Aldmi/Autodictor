                ...
                foreach (string шаблон in элементыШаблона)
                {
                    ...
                    switch (шаблон)
                    {
                        ...
                        case "ВРЕМЯ СТОЯНКИ":
                            ...
                            if(record.БитыАктивностиПолей == 31) //У тарнзита нет времени стоянки, занчит стоит галочка "Время стоянки будет измененно"
                            {
                                logMessage += "Стоянка: Время стоянки будет измененно";
                                воспроизводимыеСообщения.Add(new ВоспроизводимоеСообщение
                                {
                                    ИмяВоспроизводимогоФайла = "Время стоянки будет изменено",
                                    ТипСообщения = типСообщения,
                                    Язык = язык,
                                    ParentId = формируемоеСообщение.Id,
                                    RootId = формируемоеСообщение.SoundRecordId,
